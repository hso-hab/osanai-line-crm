...
try{const raw=CRMStore.getItem(KEY);customers=raw?JSON.parse(raw):seed();if(!Array.isArray(customers)||!customers.every(c=>c&&typeof c.id==='string'&&typeof c.name==='string'&&Object.hasOwn(labels,c.status)&&typeof c.owner==='string'&&typeof c.due==='string'&&typeof c.next==='string'&&Array.isArray(c.history)&&c.history.every(h=>typeof h.text==='string'&&Number.isFinite(Date.parse(h.at)))))throw Error('Invalid demo data');customers=customers.map(c=>({...c,transactions:c.transactions??[],factoringCases:c.factoringCases??[],legalNotices:c.legalNotices??[]}));if(!customers.every(c=>Array.isArray(c.transactions)&&c.transactions.every(t=>typeof t.id==='string'&&Number.isSafeInteger(t.revenue)&&Number.isSafeInteger(t.cost)&&(()=>{try{F.validate(t,'9999-12-31');return true}catch{return false}})())))throw Error('Invalid transaction data');if(!customers.every(c=>Array.isArray(c.factoringCases)&&c.factoringCases.every(a=>{try{window.CRMFactoring.validate(a,'9999-12-31');return true}catch{return false}})))throw Error('Invalid factoring data');}catch{customers=seed();loadError=true;}
let quick='';let selectedId=null;let toastTimer;let cleanState='';
function notify(t){window.CRMUsability?.feedback(t);$('#toast').textContent=t;clearTimeout(toastTimer);toastTimer=setTimeout(()=>{$('#toast').textContent=''},4500)}
async function persist(next){try{if(loadError)throw Error("保存データが破損しています。バックアップから復元してください。");if(window.CRMAdsUI){const work=CRMAdsUI.syncCustomers(next);if(work){await CRMWorkbench.restore(next,work);customers=next;return true;}}window.CRMWorkbench?.validateCustomers(next);await CRMStore.setItem(KEY,JSON.stringify(next));customers=next;return true}catch(error){notify(error.message||'保存できませんでした。ブラウザの保存設定をご確認ください。');return false}}
...
function render(){
 ...
 const query=$('#search').value.trim();const status=$('#status-filter').value;const owner=$('#owner-filter').value;
 const listInfo=new Map(customers.map(c=>[c.id,CRMUsability.info(c)]));
 const rows=customers.filter(c=>CRMUsability.matches(listInfo.get(c.id))&&(status==='all'||c.status===status)&&(owner==='all'||c.owner===owner)&&(!quick||quick==='due'&&dueNow(c)||quick==='unassigned'&&!c.owner&&c.status!=='done')&&CRMSearch.matches(c,query));
 rows.sort((a,b)=>CRMUsability.compare(a,b,listInfo));CRMUsability.renderFilters();
 ...
 $('#customer-list').innerHTML=rows.length?rows.map(c=>{const m=CRMManagement.customer(c),late=c.status!=='done'&&c.due&&c.due<today(),dueToday=c.status!=='done'&&c.due===today(),recoveryLate=CRMManagement.recovery([c]).some(r=>r.bucket==='late');return `<article class="customer-item ${late||recoveryLate?'is-overdue':dueToday?'is-today':''}"><button class="customer-row" data-customer="${esc(c.id)}" aria-label="${esc(c.name)}の顧客詳細を開く"><span class="customer-name"><span class="avatar" aria-hidden="true">${esc(c.name.slice(0,1))}</span><span><strong>${esc(c.name)}</strong><small class="mg-rank ${m.rank}">${CRMManagement.ranks[m.rank]}</small><span class="customer-flags">${late?'<span class="due-badge late">対応日超過</span>':dueToday?'<span class="due-badge today">今日対応</span>':''}${recoveryLate?'<span class="due-badge late">回収期限超過</span>':''}</span></span></span><span class="status-cell"><span class="status ${esc(c.status)}">${esc(labels[c.status])}</span></span><span class="owner-cell">${esc(c.owner||'担当未設定')}</span><span class="money-cell"><small>見込利益</small><strong class="${m.profit<0?'negative':''}">${yen(m.profit)}</strong></span><span class="balance-cell"><small>未回収額</small><strong>${yen(m.outstanding)}</strong></span><span class="next-cell"><strong>${esc(c.status==='done'?'対応完了':c.next||'未設定')}</strong><span class="due-date ${c.status!=='done'&&c.due&&c.due<today()?'overdue':''}">${esc(c.status==='done'?'予定なし':c.due||'日付未設定')}</span></span><span class="row-arrow" aria-hidden="true">›</span></button>${CRMQuick.actions(c)}</article>`}).join(''):(customers.length?'<div class="empty"><strong>この条件に該当するお客様はいません</strong><p>検索・絞り込みを解除すると、ほかのお客様を確認できます。</p><button type="button" class="fc-secondary" data-reset-customers>条件をすべて解除</button></div>':'<div class="empty"><strong>顧客はまだ登録されていません</strong><p>「LINE流入を体験」で架空の顧客を追加できます。</p><button type="button" class="primary" data-add-demo>＋ 架空の顧客を追加</button><p><a href="backup.html">バックアップから復元する →</a></p></div>');
...
function openCustomer(id){const c=customers.find(c=>c.id===id);if(!c)return;selectedId=id;
 ...
 const form=$('#customer-form');form.elements.customerName.value=c.name;form.elements.phone.value=c.phone||'';form.elements.lineName.value=c.lineName||'';for(const k of ['status','owner','due','next'])form.elements[k].value=c[k];form.elements.note.value='';
 renderHistory(c);
 renderCustomerFinance(c);window.dispatchEvent(new CustomEvent('crm-customer-open',{detail:{id:c.id}}));cleanState=formState();if(!$('#detail').open)$('#detail').showModal();
}
...
$('#customer-form').addEventListener('submit',async e=>{e.preventDefault();const c=customers.find(c=>c.id===selectedId);if(!c)return;const form=e.currentTarget;const status=form.elements.status.value;const owner=form.elements.owner.value;const due=form.elements.due.value;const next=form.elements.next.value.trim();const note=form.elements.note.value.trim();
 if(status!=='done'&&Boolean(due)!==Boolean(next)){CRMUsability.error(due?form.elements.next:form.elements.due,'次回対応日と「次にすること」はセットで入力してください。');(due?form.elements.next:form.elements.due).focus();return;}
 ...
 const customerRank=form.elements.customerRank.value,lastContactDate=form.elements.lastContactDate.value,channel=form.elements.acquisitionChannel.value;
 try{CRMManagement.validateProfile({customerRank,lastContactDate});}catch(err){CRMUsability.error(form.elements.lastContactDate,err.message);return;}
 if(customerRank!==(c.customerRank||'normal'))changes.push('顧客ランク（手動）：'+CRMManagement.ranks[customerRank]);if(lastContactDate!==(c.lastContactDate||''))changes.push('最終連絡日：'+(lastContactDate||'未記録'));if(channel!==(c.acquisition?.channel||'不明'))changes.push('流入経路：'+channel);
 const attribution=window.CRMAdsUI?CRMAdsUI.profileValue(c,form):c.acquisition; if(JSON.stringify(attribution)!==JSON.stringify(c.acquisition))changes.push('LP・媒体・送客日を変更');
 const contact={name:form.elements.customerName.value.trim(),phone:form.elements.phone.value.trim(),lineName:form.elements.lineName.value.trim()};if(!contact.name){CRMUsability.error(form.elements.customerName,'名前を入力してください。');return;}for(const k of ['name','phone','lineName'])if(contact[k]!==String(c[k]||''))changes.push(({name:'名前',phone:'電話番号',lineName:'LINE名'})[k]+'を変更');if(!changes.length&&!note){notify('変更はありません。');return;}
 const updated={...c,...contact,...(window.CRMServer?{ownerId:CRMServer.directory.find(u=>u.name===owner)?.id||null}:{}),customerRank,lastContactDate,acquisition:{...(attribution||{}),channel},status,owner,due:newDue,next:newNext,history:[...c.history,...(changes.length?[{at:stamp(),text:changes.join('\n')}]:[]),...(note?[{at:stamp(),text:'メモ：'+note,kind:'memo'}]:[])]};if(!await persist(customers.map(item=>item.id===c.id?updated:item)))return;render();openCustomer(c.id);notify('変更を保存しました。お客様への送信はありません。');
});
for(const name of ['search','status-filter','owner-filter'])$('#'+name).addEventListener(name==='search'?'input':'change',render);
function setView(view){CRMUsability.reset();quick=['due','unassigned'].includes(view)?view:'';$('#status-filter').value=['new','active'].includes(view)?view:'all';$('#owner-filter').value='all';$('#search').value='';render()}
...
$('#simulate').onclick=async ()=>{const number=Math.max(0,...customers.map(c=>Number(c.id.replace('DEMO-',''))||0))+1;const id='DEMO-'+String(number).padStart(3,'0');const c={id,name:'新しいお客様（サンプル）',transactions:[],status:'new',owner:'',due:'',next:'',message:'はじめまして。サービスについて相談したいです。',created:stamp(),history:[{at:stamp(),text:'LINEから初回のお問い合わせを受信（デモ操作）'}]};if(!await persist([c,...customers]))return;setView('new');openCustomer(id);notify('架空のお客様を追加しました。LINEには接続していません。');};
$('#reset').onclick=async ()=>{if(!confirm('このブラウザのデモ顧客・取引・審査案件・入金・通知・変更履歴を消して、最初のサンプルに戻しますか？'))return;if(await persist(seed())){reportAnchor=F.today();$('#report-date').value=reportAnchor;setView('all');notify('デモを最初の状態に戻しました。')}};
...
function renderSales(){
 ...
 const max=Math.max(1,...series.flatMap(p=>[p.revenue,p.profit]));const min=Math.min(0,...series.map(p=>p.profit));const height=210,top=22,bottom=165;const y=v=>top+(max-v)/(max-min)*(bottom-top);const zero=y(0),w=Math.max(240,Math.min(760,$('#sales-chart').clientWidth||640)),left=76,right=8,step=(w-left-right)/series.length;
 const ticks=[max,0,...(min<0?[min]:[])];const compact=CRMUsability.axisAmount;
...
function refreshCustomerAfterSale(){if(selectedId&&$('#detail').open){const c=customers.find(c=>c.id===selectedId);renderCustomerFinance(c);renderHistory(c);}}
function initFinance(){
 ...
 $('#sale-form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget;const customer=customers.find(c=>c.id===f.elements.customer.value);if(!customer){$('#sale-error').textContent='顧客を選択してください。';return;}
 ...
 if(!confirm(`${customer.name} の取引を${existing?'変更':'保存'}しますか？\n売上日：${tx.date}\n売上：${yen(tx.revenue)}\n費用：${yen(tx.cost)}\n利益：${yen(tx.revenue-tx.cost)}`))return;
 ...
 if(!await persist(next))return;render();refreshCustomerAfterSale();$('#sale-dialog').close();saleEditId=null;notify('取引を保存し、売上・利益・グラフを更新しました。');
 }catch(error){$('#sale-error').textContent=error.message;}};
 $('#customer-transactions').onclick=async e=>{const edit=e.target.closest('[data-edit-sale]');if(edit){openSale(selectedId,edit.dataset.editSale);return;}const btn=e.target.closest('[data-toggle-sale]');if(!btn)return;const c=customers.find(c=>c.id===selectedId),t=c.transactions.find(t=>t.id===btn.dataset.toggleSale);if(!t)return;const restoring=t.status==='void';if(!confirm(`${c.name}\n${restoring?'この取引を復元して、集計に戻しますか？':'この取引を取り消して、集計から除外しますか？ 記録は残ります。'}\n売上日：${t.date}\n売上：${yen(t.revenue)} / 費用：${yen(t.cost)}`))return;
 const next=customers.map(x=>x.id===c.id?{...x,transactions:x.transactions.map(item=>item.id===t.id?{...item,status:restoring?'posted':'void',updated:stamp()}:item),history:[...x.history,{at:stamp(),text:(restoring?'取引を復元：':'取引を取消：')+t.date+' / 売上 '+yen(t.revenue)+' / 費用 '+yen(t.cost)}]}:x);if(await persist(next)){render();refreshCustomerAfterSale();notify(restoring?'取引を復元しました。':'取引を取り消しました。');}};
}
