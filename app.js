...
function persist(next){try{window.CRMWorkbench?.validateCustomers(next);localStorage.setItem(KEY,JSON.stringify(next));customers=next;return true}catch(error){notify(error.message||'保存できませんでした。ブラウザの保存設定をご確認ください。');return false}}
...
$('#customer-form').addEventListener('submit',e=>{e.preventDefault();const c=customers.find(c=>c.id===selectedId);if(!c)return;const form=e.currentTarget;const status=form.elements.status.value;const owner=form.elements.owner.value;const due=form.elements.due.value;const next=form.elements.next.value.trim();const note=form.elements.note.value.trim();
 ...
 const updated={...c,customerRank,lastContactDate,acquisition:{...(c.acquisition||{}),channel},status,owner,due:newDue,next:newNext,history:[...c.history,{at:stamp(),text:changes.join('\n')}]};if(!persist(customers.map(item=>item.id===c.id?updated:item)))return;render();openCustomer(c.id);notify('変更を保存しました。お客様への送信はありません。');
});
...
$('#reset').onclick=()=>{if(!confirm('このブラウザのデモ顧客・取引・審査案件・入金・通知・変更履歴を消して、最初のサンプルに戻しますか？'))return;if(persist(seed())){reportAnchor=F.today();$('#report-date').value=reportAnchor;setView('all');notify('デモを最初の状態に戻しました。')}};
