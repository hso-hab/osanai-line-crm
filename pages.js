...
(()=>{
 ...
 const header=document.getElementById('ops-manual-toggle');header.hidden=page==='customers'||page==='today';header.textContent='顧客・案件を管理';
 ...
 const special={'ops-manual-toggle':'customers.html','ops-notice-link':'notices.html','ops-overdue-link':'customers.html?status=late','wb-nav':'tools.html'};
 ...
 if(page==='today')setView('due');if(page==='customers'&&new URLSearchParams(location.search).get('status')==='late'){document.getElementById('fc-status-filter').value='late';document.getElementById('fc-status-filter').dispatchEvent(new Event('change'));}
