...
function renderFactoring(){
 ...
 $('#fc-recovery').innerHTML=metric('回収率（金額）',pct(r.rate),yen(r.collected)+' / '+yen(r.expected))+metric('対象の未回収額',yen(r.remaining),r.count+'案件 / '+r.customers+'顧客')+metric('延滞残額',yen(r.overdue),r.overdueCount+'件・期日が昨日以前')+metric('期限前の未回収額',yen(r.future),'延滞には含めません');
 $('#fc-status-caption').textContent=`${o.start||'開始日指定なし'} 〜 ${o.end||'終了日指定なし'}に申し込んだ案件の、現在の審査状況・${today()}時点の入金を集計。`;
 $('#fc-rate-caption').textContent=(o.scope==='due'?'期日が今日以前の買取済案件':'期限前も含む全買取済案件')+'が対象。回収率＝回収済額 ÷ 請求書額面（回収予定額）。対象0円は「—」。';
...
function renderReceipts(c){
 ...
 $('#fc-receipt-summary').textContent=c?`未回収額 ${yen(c.invoiceAmount-C.paid(c))} ／ 回収済額 ${yen(C.paid(c))} / 予定 ${yen(c.invoiceAmount)}（請求書額面）`:'';
 if(c)f.elements.amount.max=Math.max(0,c.invoiceAmount-C.paid(c));
 $('#fc-receipts').innerHTML=(c?.receipts||[]).map(r=>`<article class="transaction ${r.void?'void':''}"><strong>${esc(r.date)} · ${yen(r.amount)}</strong><p>${esc(r.memo)} ${r.void?'（取消済）':''}</p><div class="transaction-actions">${!r.void?`<button data-receipt-edit="${esc(r.id)}">編集</button>`:''}<button data-receipt-toggle="${esc(r.id)}">${r.void?'復元':'取消'}</button></div></article>`).join('')||'<p class="fc-note">入金記録はまだありません。下の欄から記録できます。</p>';
...
$('#fc-toggle').onclick=async ()=>{const c=find(edit);if(!c||!confirm(`${c.customerName} / ${c.reference||c.id}\n${c.void?'案件を復元して集計に戻しますか？':'案件を取消して集計から除外しますか？'}\n請求書額面：${yen(c.invoiceAmount)}\n入金履歴は残り、復元できます。`))return;const next={...c,void:!c.void};try{C.validate(next);if(await update(c.id,next,next.void?'案件を取消（入金履歴を保持）':'案件を復元')){openCase(c.id);notify(next.void?'案件を取消しました。履歴は残っています。':'案件を復元しました。');}}catch(err){$('#fc-error').textContent=err.message;}};
$('#fc-receipt-form').onsubmit=async e=>{e.preventDefault();const c=find(edit),f=e.currentTarget;if(!c||c.void||c.status!=='purchased')return;try{
 ...
 if(!confirm(`${c.customerName} / ${c.reference||c.id}\n入金を${prior?'変更':'記録'}しますか？\n入金日：${r.date}\n${prior?'変更前：'+yen(prior.amount)+' → ':''}入金額：${yen(r.amount)}\n保存後の未回収額：${yen(c.invoiceAmount-C.paid(next))}\n※入金記録のみ。送金は行いません。`))return;
...
$('#fc-receipts').onclick=async e=>{const c=find(edit),b=e.target.closest('[data-receipt-edit],[data-receipt-toggle]');if(!b||!c)return;const id=b.dataset.receiptEdit||b.dataset.receiptToggle,r=c.receipts.find(r=>r.id===id);if(!r)return;
 if(b.dataset.receiptEdit){if(r.bankRowId){notify('銀行照合から登録した入金は、取消してから入金照合でやり直してください。');return;}receiptEdit=id;const f=$('#fc-receipt-form');for(const k of ['date','amount','memo'])f.elements[k].value=r[k];f.elements.amount.max=c.invoiceAmount-C.paid(c)+r.amount;$('#fc-receipt-save').textContent='入金の変更を保存';f.scrollIntoView({block:'nearest',behavior:'smooth'});return;}
 if(!confirm(`${c.customerName} / ${c.reference||c.id}\nこの入金を${r.void?'復元':'取消'}しますか？\n入金日：${r.date}\n入金額：${yen(r.amount)}\n変更後の未回収額：${yen(c.invoiceAmount-C.paid(c)+(r.void?-r.amount:r.amount))}\n記録は残ります。`))return;
 const next={...c,receipts:c.receipts.map(x=>x.id===id?{...x,void:!x.void}:x)};try{C.validate(next);if(await update(c.id,next,`入金${r.void?'復元':'取消'}：${r.date} / ${yen(r.amount)}`)){renderReceipts(find(c.id));$('#fc-dialog-title').textContent=c.id+' · '+C.state(find(c.id));$('#fc-receipt-error').textContent='';notify(r.void?'入金を復元しました。':'入金を取消しました。履歴は残っています。');}}catch(err){$('#fc-receipt-error').textContent=err.message;}
};
