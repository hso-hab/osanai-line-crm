...
function render(){
 ...
 $('#customer-list').innerHTML=rows.length?rows.map(c=>{const m=CRMManagement.customer(c);return `<button class="customer-row" data-customer="${esc(c.id)}" aria-label="${esc(c.name)}の顧客詳細を開く"><span class="customer-name"><span class="avatar" aria-hidden="true">${esc(c.name.slice(0,1))}</span><span><strong>${esc(c.name)}</strong><small class="mg-rank ${m.rank}">${CRMManagement.ranks[m.rank]}</small></span></span><span class="status-cell"><span class="status ${esc(c.status)}">${esc(labels[c.status])}</span></span><span class="owner-cell">${esc(c.owner||'担当未設定')}</span><span class="money-cell"><small>見込利益</small><strong class="${m.profit<0?'negative':''}">${yen(m.profit)}</strong></span><span class="balance-cell"><small>未回収</small><strong>${yen(m.outstanding)}</strong></span><span class="next-cell"><strong>${esc(c.status==='done'?'対応完了':c.next||'未設定')}</strong><span class="due-date ${c.status!=='done'&&c.due&&c.due<today()?'overdue':''}">${esc(c.status==='done'?'予定なし':c.due||'日付未設定')}</span></span><span class="row-arrow" aria-hidden="true">›</span></button>`}).join(''):'<div class="empty"><strong>該当するお客様はいません</strong>検索条件や対応状況を変更してください。</div>';
...
$('#customer-form').addEventListener('submit',async e=>{e.preventDefault();const c=customers.find(c=>c.id===selectedId);if(!c)return;const form=e.currentTarget;const status=form.elements.status.value;const owner=form.elements.owner.value;const due=form.elements.due.value;const next=form.elements.next.value.trim();const note=form.elements.note.value.trim();
 ...
 const contact={name:form.elements.customerName.value.trim(),phone:form.elements.phone.value.trim(),lineName:form.elements.lineName.value.trim()};if(!contact.name){notify('名前を入力してください。');return;}for(const k of ['name','phone','lineName'])if(contact[k]!==String(c[k]||''))changes.push(({name:'名前',phone:'電話番号',lineName:'LINE名'})[k]+'を変更');if(note)changes.push('メモ：'+note);if(!changes.length){notify('変更はありません。');return;}
 const updated={...c,...contact,...(window.CRMServer?{ownerId:CRMServer.directory.find(u=>u.name===owner)?.id||null}:{}),customerRank,lastContactDate,acquisition:{...(c.acquisition||{}),channel},status,owner,due:newDue,next:newNext,history:[...c.history,{at:stamp(),text:changes.join('\n')}]};if(!await persist(customers.map(item=>item.id===c.id?updated:item)))return;render();openCustomer(c.id);notify('変更を保存しました。お客様への送信はありません。');
});
