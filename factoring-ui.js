...
function renderReceipts(c){
 ...
 $('#fc-receipt-summary').textContent=c?`未回収 ${yen(c.invoiceAmount-C.paid(c))} ／ 回収済 ${yen(C.paid(c))} / 予定 ${yen(c.invoiceAmount)}（請求書額面）`:'';
...
$('#fc-toggle').onclick=async ()=>{const c=find(edit);if(!c||!confirm(c.void?'案件を復元して集計に戻しますか？':'案件を取消して集計から除外しますか？ 入金履歴は残り、復元できます。'))return;const next={...c,void:!c.void};try{C.validate(next);if(await update(c.id,next,next.void?'案件を取消（入金履歴を保持）':'案件を復元')){openCase(c.id);notify(next.void?'案件を取消しました。履歴は残っています。':'案件を復元しました。');}}catch(err){$('#fc-error').textContent=err.message;}};
