...
function priority(a,b){const asOf=F.today(),group=c=>c.status==='done'?4:!c.due?3:c.due<asOf?0:c.due===asOf?1:2;return group(a)-group(b)||(a.due||'9999').localeCompare(b.due||'9999')||Number(b.status==='new')-Number(a.status==='new')||a.id.localeCompare(b.id);}
const api={priority,ranks,comparison,change,stats,customer,recovery,analysis,validateProfile};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.CRMManagement=api;
