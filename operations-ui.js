...
function drawChart(series){
 const width=Math.max(250,$('#ops-chart').clientWidth||640),h=220,left=76,right=10,top=22,bottom=175;const max=Math.max(1,...series.map(v=>v.fee)),min=Math.min(0,...series.map(v=>v.profit));const y=v=>top+(max-v)/(max-min)*(bottom-top),step=(width-left-right)/series.length,bw=Math.min(22,step*.26);const zero=y(0);const compact=CRMUsability.axisAmount;
...
async function saveNotice(cid,n,description){const c=customers.find(c=>c.id===cid);if(!c)return false;const previous=(c.legalNotices||[]).find(x=>x.id===n.id),record={...n,history:[...(previous?.history||[]),{at:stamp(),text:description}]};const next=customers.map(c=>c.id===cid?{...c,legalNotices:previous?c.legalNotices.map(x=>x.id===n.id?record:x):[...(c.legalNotices||[]),record],history:[...c.history,{at:stamp(),text:description}]}:c);if(!await persist(next))return false;render();refreshCustomerAfterSale();return true;}
...
$('#notice-form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,cid=f.elements.noticeCustomer.value,c=customers.find(c=>c.id===cid),previous=c?.legalNotices?.find(n=>n.id===noticeId);if(!c)return;try{if(noticeId&&(!previous||previous.void))throw Error('通知の状態を確認してください。');const n={id:previous?.id||'NT-'+crypto.randomUUID(),void:false,memo:f.elements.noticeMemo.value.trim()};for(const k of fields)n[k]=f.elements[k].value.trim();O.validateNotice(n);
 ...
 if(await saveNotice(cid,n,desc)){$('#notice-dialog').close();notify('通知を記録しました。審査結果は変更せず、責任者確認の対象にしました。');}
...
$('#notice-action-form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,c=customers.find(c=>c.id===action?.cid),n=c?.legalNotices.find(n=>n.id===action.id);if(!n)return;const actor=f.elements.actor.value.trim(),reason=f.elements.reason.value.trim();if(!actor||!reason){$('#notice-action-error').textContent='担当者と理由を入力してください。';return;}if(await saveNotice(c.id,{...n,void:!n.void},`通知${n.void?'復元':'取消'}：${O.noticeKinds[n.kind]} / 担当 ${actor} / 理由 ${reason}`)){$('#notice-action-dialog').close();notify('通知の履歴と集計を更新しました。');}};
