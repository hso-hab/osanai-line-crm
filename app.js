...
async function persist(next){try{window.CRMWorkbench?.validateCustomers(next);await CRMStore.setItem(KEY,JSON.stringify(next));customers=next;return true}catch(error){notify(error.message||'保存できませんでした。ブラウザの保存設定をご確認ください。');return false}}
...
$('#customer-form').addEventListener('submit',async e=>{e.preventDefault();const c=customers.find(c=>c.id===selectedId);if(!c)return;const form=e.currentTarget;const status=form.elements.status.value;const owner=form.elements.owner.value;const due=form.elements.due.value;const next=form.elements.next.value.trim();const note=form.elements.note.value.trim();
 ...
 const updated={...c,...contact,...(window.CRMServer?{ownerId:CRMServer.directory.find(u=>u.name===owner)?.id||null}:{}),customerRank,lastContactDate,acquisition:{...(c.acquisition||{}),channel},status,owner,due:newDue,next:newNext,history:[...c.history,...(changes.length?[{at:stamp(),text:changes.join('\n')}]:[]),...(note?[{at:stamp(),text:'メモ：'+note,kind:'memo'}]:[])]};if(!await persist(customers.map(item=>item.id===c.id?updated:item)))return;render();openCustomer(c.id);notify('変更を保存しました。お客様への送信はありません。');
});
